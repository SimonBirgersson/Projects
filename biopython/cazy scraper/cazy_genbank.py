...
import pickle 
from bs4 import BeautifulSoup # search tool for large text files
...
soup=re.findall('<a((.|\s)+?)</a>', str(u)) # Finds all elements surrounded by <a>, i.e links etc. in cazy
...
text_file=open("genbank_Acc_codes_"+today,"w") # names and creates a file to be saved in local directory
...
webbrowser.open('http://eutils.ncbi.nlm.nih.gov/entrez/eutils/efetch.fcgi?db=protein&rettype=fasta&retmode=text&id='+output, new=2)
